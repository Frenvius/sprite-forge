...
export const ImportDialog = () => {
	...
	return (
		<Dialog open={v.importOpen} onOpenChange={(o) => (o ? null : v.closeImport())}>
			<DialogContent className="flex h-[680px] max-h-[88vh] flex-col gap-0 border-0 p-0 outline-none focus:outline-none sm:max-w-[1200px]">
				<div className="flex min-h-0 flex-1">
									...
									<div className="border-y border-border/50 bg-secondary/30 px-5 py-2">
										<div className="mb-1 flex items-center justify-between text-[11px] text-muted-foreground">
											...
											<span className="font-mono">
												{fmtTime(v.progress.elapsedMs / 1000)} · {Math.round(speed).toLocaleString()}/s · {fmtTime(remaining)}{' '}
												left
											</span>
										</div>
													...
													cells.push(
														row ? (
															<ObdCard
																...
																key={pos}
																onToggle={v.toggle}
																onFocus={v.setFocusedRow}
																thumb={v.getThumb(row.recordIndex)}
																selected={v.selected.has(row.recordIndex)}
																focused={v.focusedRow?.recordIndex === row.recordIndex}
															/>
														) : (
					...
					<aside className="hidden w-[384px] shrink-0 border-l border-border/50 sm:flex sm:flex-col">
						<ImportPreview row={v.focusedRow} transparency={v.transparency} />
					</aside>
				</div>
